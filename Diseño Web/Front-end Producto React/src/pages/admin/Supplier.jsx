...
const Supplier = () => {
  ...
  const [values, handleChangeValues, resetForm] = useForm({
    rut: "",
    companyName: "",
    address: "",
    phone: "",
  ...
  const handleCreateSupplier = async (e) => {
    ...
    try {
      ...
      const lastIdOfSupplier =
        parseInt(suppliers[suppliers.length - 1].id_supplier) + 1;
      setSuppliers([
        ...suppliers,
        {
          rut: values.rut,
  ...
  const handleDeleteSupply = async (idSupplier) => {
    const confirm = window.confirm(
      "¿Estas seguro que desas borrar el proveedor?"
    );
    if (!confirm) return;
  ...
  const handleEnableSupply = async (idSupplier) => {
    const confirm = window.confirm(
      "¿Estas seguro que desas activar el proveedor?"
    );
    if (!confirm) return;
    const resp = await fetchApi(
  ...
  useEffect(() => {
    fetchApi("suppliers.php?all", "GET")
      ...
      .finally(() =>
        setLoadingFlags({ ...loadingFlags, fetchingSuppliers: false })
      );
  }, []);
          ...
          <>
            ...
            <div>
              <form autoComplete="off" onSubmit={handleCreateSupplier}>
                ...
                <button
                  className={`button-form ${loadingFlags.createSupplier &&
                    "opacity"}`}
                  disabled={loadingFlags.createSupplier}
                >
                  {loadingFlags.createSupplier
                    ? "Cargando..."
                    : "CREAR PROVEEDOR"}
                </button>
                {error.showMessage && (
                    ...
                    {suppliers.map((supplier) => (
                      <tr key={supplier.id_supplier}>
                        ...
                        <td
                          ...
                          onClick={() =>
                            navigate(
                              `/admin/generals/supplier/edit/${supplier.rut}`
                            )
                          }
                        >
                        ...
                        {supplier.state === "1" ? (
                          <td
                            className="controls-table"
                            onClick={() =>
                              handleDeleteSupply(supplier.id_supplier)
                            }
                          >
                          ...
                          <td
                            ...
                            onClick={() =>
                              handleEnableSupply(supplier.id_supplier)
                            }
                          >
